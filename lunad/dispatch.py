...
class Dispatcher:
    ...
    def agent_bin(self) -> str:
        ...
        return agent_mod.ClaudeAdapter().binary()
    ...
    def _runner_script(self, job: Job, timeout: float, linger: float) -> str:
        ...
        agent = shlex.quote(self.agent_bin())
        ...
        extra_dir = (f" \\\n    --add-dir {shlex.quote(str(self.sol_memory_dir))}"
                     if job.to == "sol" else "")
        return f"""#!/bin/bash
...
timeout --signal=TERM --kill-after=20 {int(timeout)} \\
  {agent} -p \\
    --append-system-prompt "$(cat "$JOB/system.txt")" \\
    --permission-mode bypassPermissions \\
    --tools default \\
    --safe-mode \\
    --add-dir "$JOB"{extra_dir} \\
    < "$JOB/task.txt" \\
    2> "$JOB/stderr.txt" \\
  | tee "$JOB/output.txt"
rc=$?
